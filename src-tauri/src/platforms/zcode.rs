...
impl ZCodePlatform {
    ...
    fn connect(&self) -> Result<rusqlite::Connection, String> {
        ...
        conn.busy_timeout(std::time::Duration::from_millis(800)).ok();
        Ok(conn)
    }
    ...
    fn searchable_content_entries(&self, session_key: &str) -> Vec<SessionContentEntry> {
        ...
        while let Ok(Some(row)) = rows.next() {
            ...
            if kind == "tool" {
                if let Some(name) = tool_name_from_part(&data) {
...
fn top_level_session_filter(conn: &rusqlite::Connection) -> String {
    let has_task_type = conn
        .prepare("SELECT 1 FROM pragma_table_info('session') WHERE name = 'task_type'")
        .ok()
        .and_then(|mut stmt| stmt.exists([]).ok())
        .unwrap_or(false);
    if has_task_type {
        "(parent_id IS NULL OR parent_id = '') AND (task_type IS NULL OR task_type = '' OR task_type != 'subagent_child')"
            .to_string()
    } else {
        "(parent_id IS NULL OR parent_id = '')".to_string()
    }
}

pub fn default_zcode_db_path() -> PathBuf {
    dirs::home_dir()
...
impl super::PlatformAdapter for ZCodePlatform {
    fn list_sessions(
        ...
        // Get total count
        let session_filter = top_level_session_filter(&conn);
        let total: usize = conn
            .query_row(
                &format!("SELECT COUNT(*) FROM session WHERE {session_filter}"),
                [],
                |row| row.get(0),
            )
            .unwrap_or(0);
        ...
        let sql = match limit {
            Some(l) => format!(
                "SELECT id, title, directory, time_updated FROM session WHERE {session_filter} ORDER BY time_updated DESC LIMIT {l} OFFSET {offset}"
            ),
            None => format!(
                "SELECT id, title, directory, time_updated FROM session WHERE {session_filter} ORDER BY time_updated DESC LIMIT -1 OFFSET {offset}"
            ),
        };
    ...
    fn get_session_detail(
        ...
        let mut stmt = conn.prepare(
            "SELECT part.id, part.data, message.data as message_data FROM part JOIN message ON message.id = part.message_id WHERE part.session_id = ?1 ORDER BY part.sequence ASC, part.time_created ASC, part.id ASC"
    ...
    fn replace_tool_call(
        ...
        let tx = conn
            .transaction()
            .map_err(|e| format!("Cannot start ZCode transaction: {e}"))?;
        ...
        let current = tx
            ...
            .optional()
            .map_err(|e| format!("Cannot read ZCode tool part: {e}"))?;
        ...
        match record {
            None => {
                let Some(_) = current.as_ref() else {
                    return Err("ZCode tool part was not found in this session".to_string());
                };
                tx.execute(
                    "DELETE FROM part WHERE id = ?1 AND session_id = ?2",
                    params![tool_call_id, session_key],
                )
                .map_err(|e| format!("Cannot erase ZCode tool part: {e}"))?;
            }
            Some(serialized) => {
                let stored: StoredPart = serde_json::from_str(serialized)
                    .map_err(|e| format!("Invalid ZCode tool restore record: {e}"))?;
                if stored.id != tool_call_id || stored.session_id != session_key {
                    return Err(
                        "ZCode tool restore record does not belong to this session".to_string(),
                    );
                }
                ...
                let existing_owner: Option<String> = tx
                    ...
                    .optional()
                    .map_err(|e| format!("Cannot validate ZCode tool owner: {e}"))?;
                if existing_owner
                    .as_deref()
                    .is_some_and(|owner| owner != session_key)
                {
                    return Err(
                        "An ZCode part with this ID belongs to another session".to_string()
                    );
                }
                let message_belongs_to_session: bool = tx
                    ...
                    .map_err(|e| format!("Cannot validate ZCode tool message: {e}"))?;
                if !message_belongs_to_session {
                    return Err("The original ZCode message no longer exists".to_string());
                }
                tx.execute(
                ...
                .map_err(|e| format!("Cannot restore ZCode tool part: {e}"))?;
            }
        }

        tx.commit()
            .map_err(|e| format!("Cannot commit ZCode tool change: {e}"))?;
        current
    ...
    fn warm_content_index(
        ...
        if index.is_current("zcode", session_key, &fingerprint) {
            return true;
        }
        let entries = self.searchable_content_entries(session_key);
        index
            .replace("zcode", session_key, &fingerprint, &entries)
            .is_ok()
    }
    ...
    fn has_current_content_index(
        ...
        index.is_current("zcode", session_key, &fingerprint)
    }
    ...
    fn content_search_with_index(
        ...
        if let Some(entries) = index.get_matches("zcode", session_key, &fingerprint, &needle) {
            return content_entries_to_matches(entries, &needle);
        }
        let entries = self.searchable_content_entries(session_key);
        let matches = content_entries_to_matches(entries.clone(), &needle);
        let _ = index.replace("zcode", session_key, &fingerprint, &entries);
        matches
    }
}
...
fn ensure_tool_part(data: &str) -> Result<(), String> {
    let payload: Value =
        serde_json::from_str(data).map_err(|e| format!("Invalid ZCode part data: {e}"))?;
    if payload.get("type").and_then(Value::as_str) != Some("tool") {
        return Err("The selected ZCode part is not a tool call".to_string());
    }
    Ok(())
}
...
fn tool_part_to_block(part_id: &str, data: &Value) -> Option<ToolCallBlock> {
    ...
    Some(ToolCallBlock {
        ...
        error: state
            ...
            .and_then(|value| tool_text_from_value(value, 8192)),
        started_at: tool_time_field(state, data, "start", "time_start"),
        ended_at: tool_time_field(state, data, "end", "time_end"),
        source_meta: serde_json::json!({"partType": kind}),
    })
}

fn tool_name_from_part(data: &Value) -> Option<&str> {
    data.get("tool")
        .and_then(Value::as_str)
...
fn tool_time_field(
    ...
    state
        .and_then(|value| value.pointer(&format!("/time/{nested_key}")))
        .or_else(|| state.and_then(|value| value.get(flat_key)))
        .or_else(|| data.get(flat_key))
        .and_then(|value| {
            value
                .as_i64()
                .map(|n| n.to_string())
...
mod tests {
    ...
    fn test_db(label: &str) -> PathBuf {
        ...
        let dir = base.join(format!("zcode-tool-{label}-{}", Uuid::new_v4()));
        fs::create_dir_all(&dir).expect("create test directory");
        let path = dir.join("zcode.db");
    ...
    fn tool_part_to_block_extracts_name_input_output_and_status() {
        ...
        assert_eq!(tool_call.started_at.as_deref(), Some("10"));
        assert_eq!(tool_call.ended_at.as_deref(), Some("20"));
    }
    ...
    fn erases_rejected_tool_part_and_restores_complete_payload() {
        ...
        let platform = ZCodePlatform::new(path.clone());
